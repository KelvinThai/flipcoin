...
import { flipCoinAction, generateContract } from "../../reduxs/accounts/account.actions";
...
export default function PlayView() {
  ...
  const { walletInfo, flip, web3Provider } = useAppSelector((state) => state.account);
  ...
  const handleFlipNow = async () => {
    if (headTail === undefined || chooseBet === undefined || !web3Provider) return;
    try {
      ...
      >
        <VStack w={{ base: "100%", lg: "30%" }}>
          ...
          <HStack my="20px !important" w="full">
            ...
            <Text variant="with-title" textAlign="center">CHOOSE BET {chooseBet ? `(${numberFormat(chooseBet)}BNB)` : ''} </Text>
